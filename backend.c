...
//     const char *csv_filepath = "minidataset.csv";
...
//     char fecha[8]; // "MM/YYYY" = 7 chars + '\0'
//     snprintf(fecha, sizeof(fecha), "%02d/%04d", filter_month, filter_year);

//     unsigned int hash_index = hash_function(id_to_find, fecha) % HASH_TABLE_SIZE;
...
void perform_search(const char *id_to_find, int filter_year, int filter_month) {
    ...
    // Inicializar punteros de archivos
    FILE *index_file = NULL;
    FILE *csv_file = NULL;

    if (current_node_offset == -1) {
        goto end_search; // No hay datos para este hash
    }

    // Abrir archivos solo si hay datos que procesar
    index_file = fopen(index_filepath, "rb");
    csv_file = fopen(csv_filepath, "r");
    
    if (!index_file || !csv_file) {
        goto end_search; // Los archivos se cerrarán en end_search
    }
...
end_search:
    // Cerrar archivos solo si fueron abiertos exitosamente
    if (index_file) {
        fclose(index_file);
    }
    if (csv_file) {
